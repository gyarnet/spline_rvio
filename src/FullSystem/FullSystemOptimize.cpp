...
bool FullSystem::doStepFromBackup(float stepfacC, float stepfacT, float stepfacR, float stepfacA, float stepfacD) {
    ...
    if (setting_enable_imu && HCalib.imu_initialized) {
        ...
        for (FrameHessian *fh : frameHessians) {
            fh->setImuState(fh->state_imu_backup + pstepfac_imu.cwiseProduct(fh->step_imu));
        }
    }
    ...
    if (!setting_debugout_runquiet)
        printf("STEPS: A %.1f; B %.1f; R %.1f; T %.1f. \t", sqrtf(sumA) / (0.0005 * setting_thOptIterations), sqrtf(sumB) / (0.00005 * setting_thOptIterations),
...
float FullSystem::optimize(int mnumOptIts) {
    ...
    frameHessians.back()->setEvalPT(frameHessians.back()->PRE_camToWorld, newStateZero);
    EFDeltaValid = false;
    ...
    if (setting_enable_imu) {
        if (HCalib.imu_initialized) {
            frameHessians.back()->updateVel(frameHessians[frameHessians.size() - 2]->shell);
