...
CoarseInitializer::~CoarseInitializer() {
    ...
    delete[] JbBuffer;
    delete[] JbBuffer_new;
    for (float *ptr : ptr_to_delete_) delete[] ptr;
...
void CoarseInitializer::makeCoarseDepth() {
    ...
    for (int lvl = 2; lvl < pyrLevelsUsed; lvl++) {
        ...
        float *weightSumsl_bak = weight_sums_bak_[lvl];
        memcpy(weightSumsl_bak, weightSumsl, w_[lvl] * h_[lvl] * sizeof(float));
        float *idepthl = idepth_[lvl];  // dotnt need to make a temp copy of depth,
    ...
    for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
        ...
        if (printDebug) {
            printf(
                ...
                "(|inc| = %f)! \t",
                lvl, 0, lambda, "INITIA", sqrtf((float)(resOld[0] / resOld[2])), sqrtf((float)(resOld[1] / resOld[2])), sqrtf((float)(resOld[0] / resOld[2])),
        ...
        while (true) {
            ...
            if (printDebug) {
                printf(
                    "lvl %d, it %d (l=%f) %s: %.5f + %.5f + %.5f -> %.5f + %.5f + "
                    "%.5f (%.2f->%.2f) (|inc| = %f)! \t",
                    lvl, iteration, lambda, (accept ? "ACCEPT" : "REJECT"), sqrtf((float)(resOld[0] / resOld[2])), sqrtf((float)(regEnergy[0] / regEnergy[2])),
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper *> &wraps) {
    bool needCall = false;
    for (IOWrap::Output3DWrapper *ow : wraps) needCall = needCall || ow->needPushDepthImage();
    if (!needCall) return;
    ...
    for (int i = 0; i < wl * hl; i++) iRImg.at(i) = Vec3b(colorRef[i][0], colorRef[i][0], colorRef[i][0]);
...
// depth.
Vec3f CoarseInitializer::calcResAndGS(int lvl, Mat88f &H_out, Vec8f &b_out, Mat88f &H_out_sc, Vec8f &b_out_sc, const SE3 &refToNew, AffLight refToNew_aff,
                                      bool plot) {
    int wl = w_[lvl], hl = h_[lvl];
    Eigen::Vector3f *colorRef = firstFrame->dIp[lvl];
...
void CoarseInitializer::setFirst(CalibHessian *HCalib, FrameHessian *newFrameHessian) {
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
        ...
        int npts;
        if (lvl == 0)
            npts = sel.makeMaps(firstFrame, statusMap, densities[lvl] * w_[0] * h_[0], 1, false, 2);
        else
            npts = makePixelStatus(firstFrame->dIp[lvl], statusMapB, w_[lvl], h_[lvl], densities[lvl] * w_[0] * h_[0]);
