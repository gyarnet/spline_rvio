...
void FrameHessian::setStateZero(const Vec10 &state_zero) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        SE3 c2w_leftEps_P_x0 = (get_camToWorld_evalPT() * EepsP) * get_camToWorld_evalPT().inverse();
        SE3 c2w_leftEps_M_x0 = (get_camToWorld_evalPT() * EepsM) * get_camToWorld_evalPT().inverse();
        nullspaces_pose.col(i) = (c2w_leftEps_P_x0.log() - c2w_leftEps_M_x0.log()) / (2e-3);
    }
...
void FrameHessian::getImuHi(CalibHessian *HCalib, double tt, Mat36 &JsTW, Mat296 &JfTW, Mat33 &Hss, Mat2929 &Hff, Mat293 &Hfs) {
    assert(tt <= 0);
    double tt2 = tt * tt;
    ...
    Mat33 rot_t_w = getSplineR_c_t(tt, HCalib->scale_trapped).transpose() * get_camToWorld_evalPT().rotationMatrix().transpose();
...
bool FrameHessian::initializeImu(const std::vector<FrameHessian *> &frame_hessians, CalibHessian *HCalib) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        A(i, 1) = A(i, 0) * A(i, 0);
        A(i, 2) = A(i, 1) * A(i, 0);
        b.row(i) = (base_frame->shell->camToWorld.inverse() * cur_frame->shell->camToWorld).log();
        b.row(i).head(3) = cur_frame->shell->camToWorld.translation() - base_frame->shell->camToWorld.translation();
    }
    ...
    for (int i = 2; i < 5; i++) {
        all_imu_data.insert(all_imu_data.end(), frame_hessians[i]->imu_data.begin(), frame_hessians[i]->imu_data.end());
    }
    ...
    for (size_t i = 0; i < all_imu_data.size(); i++) {
        double t = all_imu_data[i].timestamp - base_frame->shell->timestamp;
        Mat33 rot_ti_w = setting_rot_imu_cam * base_frame->getSplineR_c_t(t).transpose() * base_frame->PRE_worldToCam.rotationMatrix();
...
void FrameHessian::propagateImuState(FrameShell *last_shell, const Vec6 &last_imu_bias, CalibHessian *HCalib) {
    ...
    for (int i = 0; i < imu_data.size(); i++) {
        ...
        // acc
        Aa.row(i) << 0, 2 * HCalib->getScaleScaled(), 6 * t * HCalib->getScaleScaled();
        ba.row(i) = imu_rot_w_ti * setting_rot_imu_cam.transpose() * unbias_acc - HCalib->getG();
        // gyro
    ...
    shell->velInWorld = last_shell->velInWorld - (2 * t * spline_q + 3 * t * t * spline_c).head(3);
}

void FrameHessian::updateVel(FrameShell *last_shell) {
    double t = last_shell->timestamp - shell->timestamp;
    Vec3 tsl_diff = last_shell->camToWorld.translation() - shell->camToWorld.translation();
    shell->velInWorld = tsl_diff / t - t * spline_q.head(3) - t * t * spline_q.head(3);
}
...
void CalibHessian::tryTrapScale() {
    ...
    double var = 1.0 / 9.0 * SCALE_SCALE * SCALE_SCALE * (scale_queue - Vec10::Constant(scale_queue.mean())).squaredNorm();
...
void FrameFramePrecalc::set(FrameHessian *host, FrameHessian *target, CalibHessian *HCalib) {
    ...
    SE3 leftToLeft_0 = target->get_camToWorld_evalPT().inverse() * host->get_camToWorld_evalPT();
