...
struct FrameHessian {
    ...
    EIGEN_STRONG_INLINE const SE3 &get_camToWorld_evalPT() const { return camToWorld_evalPT; }
    ...
    inline void setEvalPT_scaled(const SE3 &camToWorld_evalPT, const AffLight &aff_g2l) {
    ...
    EIGEN_STRONG_INLINE const Vec21 &getImuStateScaled() const { return state_imu_scaled; }
    ...
    inline void setImuStateScaled(const Vec21 &new_state_imu_scaled) {
        ...
        state_imu.segment<3>(6) = SCALE_SL_ROT_INVERSE * state_imu_scaled.segment<3>(6);
        state_imu.segment<3>(9) = SCALE_SQ_TRANS_INVERSE * state_imu_scaled.segment<3>(9);
        state_imu.segment<3>(12) = SCALE_SQ_ROT_INVERSE * state_imu_scaled.segment<3>(12);
        state_imu.segment<3>(15) = SCALE_SC_TRANS_INVERSE * state_imu_scaled.segment<3>(15);
        state_imu.segment<3>(18) = SCALE_SC_ROT_INVERSE * state_imu_scaled.segment<3>(18);
    }
    ...
    inline Vec3 getSplineAcc(double t, bool use_state_zero = false) const {
        ...
        if (!use_state_zero) {
            ...
            acc = 2 * SCALE_SQ_TRANS * state_imu_zero.segment<3>(9) + 6 * t * SCALE_SC_TRANS * state_imu_zero.segment<3>(15);
        }
        return acc;
    }

    inline Vec3 getSplineGryo(double t) const { return spline_l_rot + (2 * t * spline_q + 3 * t * t * spline_c).tail(3); }
    ...
    inline Mat33 getSplineR_c_t(double t, bool use_state_zero = false) const {
        ...
        if (!use_state_zero) {
            ...
            so3 = t * SCALE_SL_ROT * state_imu_zero.segment<3>(6) + t2 * SCALE_SQ_ROT * state_imu_zero.segment<3>(12) +
                  t * t2 * SCALE_SC_ROT * state_imu_zero.segment<3>(18);
        }
        return SO3::exp(so3).matrix();
    }

    void getImuHi(CalibHessian *HCalib, double tt, Mat36 &JsTW, Mat296 &JfTW, Mat33 &Hss, Mat2929 &Hff, Mat293 &Hfs);

    void setImuStateZero(CalibHessian *HCalib);

    static bool initializeImu(const std::vector<FrameHessian *> &frame_hessians, CalibHessian *HCalib);

    void propagateImuState(FrameShell *last_shell, const Vec6 &last_imu_bias, CalibHessian *HCalib);
...
struct CalibHessian {
    ...
    inline double getScaleScaled(bool use_state_zero = false) { return use_state_zero ? (sg_zero[0] * SCALE_SCALE) : sg_scaled[0]; }

    inline void getGSinCos(double &sr, double &cr, double &sp, double &cp, bool use_state_zero = false) {
        Vec2 rp = use_state_zero ? (sg_zero.tail(2) * SCALE_G).eval() : sg_scaled.tail(2);
        sr = std::sin(rp[0]);
        cr = std::cos(rp[0]);
        sp = std::sin(rp[1]);
        cp = std::cos(rp[1]);
