...
void CoarseTracker::makeK(CalibHessian *HCalib) {
    ...
    for (int level = 0; level < pyrLevelsUsed; ++level) {
        Mat33f K;
        K << fx_[level], 0.0, cx_[level], 0.0, fy_[level], cy_[level], 0.0, 0.0, 1.0;
        Ki_[level] = K.inverse();
    }
}
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian *> frameHessians) {
    ...
    for (int lvl = 1; lvl < pyrLevelsUsed; lvl++) {
        ...
        for (int y = 0; y < hl; y++)
            for (int x = 0; x < wl; x++) {
                ...
                weight_sums_l[x + y * wl] = weight_sums_lm[bidx] + weight_sums_lm[bidx + 1] + weight_sums_lm[bidx + wlm1] + weight_sums_lm[bidx + wlm1 + 1];
            }
    }
    ...
    for (int lvl = 0; lvl < 2; lvl++) {
        ...
        for (int it = 0; it < numIts; it++) {
            ...
            float *idepthl = idepth_[lvl];  // dotnt need to make a temp copy of depth, since I only
...
void CoarseTracker::debugPlotIDepthMap(float *minID_pt, float *maxID_pt, std::vector<IOWrap::Output3DWrapper *> &wraps) {
...

    {
        ...
        for (int i = 0; i < h_[lvl] * w_[lvl]; i++) {
            if (idepth_[lvl][i] > 0) allID.push_back(idepth_[lvl][i]);
        }
...
void CoarseTracker::debugPlotIDepthMapFloat(std::vector<IOWrap::Output3DWrapper *> &wraps) {
    ...
    for (IOWrap::Output3DWrapper *ow : wraps) ow->pushDepthImageFloat(&mim, lastRef);
}
...
bool CoarseTracker::trackNewestCoarse(FrameHessian *newFrameHessian, SE3 &lastToNew_out, AffLight &aff_g2l_out, int coarsestLvl, Vec5 minResForAbort,
                                      Vec5 &lastResiduals, IOWrap::Output3DWrapper *wrap) {
    ...
    for (int lvl = coarsestLvl; lvl >= 0; lvl--) {
        ...
        while (resOld[5] > 0.6 && levelCutoffRepeat < 50) {
            ...
            if (!setting_debugout_runquiet) printf("INCREASING cutoff to %f (ratio is %f)!\n", setting_coarseCutoffTH * levelCutoffRepeat, resOld[5]);
        }
        ...
        if (DEBUG_PRINT) {
            ...
            printf(
                ...
                "%f)! \t",
                lvl, -1, lambda, 1.0f, "INITIA", 0.0f, resOld[0] / resOld[1], 0, (int)resOld[1], 0.0f);
        ...
        for (int iteration = 0; iteration < maxIterations[lvl]; iteration++) {
            ...
            if (DEBUG_PRINT) {
                ...
                printf(
                    "lvl %d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> %d) (|inc| = "
                    "%f)! \t",
                    lvl, iteration, lambda, extrapFac, (accept ? "ACCEPT" : "REJECT"), resOld[0] / resOld[1], resNew[0] / resNew[1], (int)resOld[1],
            ...
            if (!(inc.norm() > 1e-3)) {
                if (DEBUG_PRINT) printf("inc too small, break!\n");
                break;
            }
        }
    ...
    if ((setting_affineOptModeA != 0 && (fabsf(aff_g2l_out.a) > 1.2)) || (setting_affineOptModeB != 0 && (fabsf(aff_g2l_out.b) > 200))) return false;
...
void CoarseTracker::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, const SE3 &refToNew, AffLight aff_g2l) {
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        poseAcc.updateSSE_eighted(_mm_mul_ps(id, dx), _mm_mul_ps(id, dy), _mm_sub_ps(zero, _mm_mul_ps(id, _mm_add_ps(_mm_mul_ps(u, dx), _mm_mul_ps(v, dy)))),
                                  ...
                                  _mm_sub_ps(_mm_mul_ps(u, dy), _mm_mul_ps(v, dx)), _mm_mul_ps(a, _mm_sub_ps(b0, _mm_load_ps(poseBufWarped_refColor + i))),
                                  minusOne, _mm_load_ps(poseBufWarped_residual + i), _mm_load_ps(poseBufWarped_weight + i));
    }
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH, bool plot_img) {
    float E = 0;
    int numTermsInE = 0;
    ...
    for (int i = 0; i < nl; i++) {
        ...
        if (fabs(residual) > cutoffTH) {
            if (plot_img) resImage->setPixel4(lpc_u[i], lpc_v[i], Vec3b(0, 0, 255));
            E += maxEnergy;
            numTermsInE++;
            numSaturated++;
        } else {
            if (plot_img) resImage->setPixel4(lpc_u[i], lpc_v[i], Vec3b(residual + 128, residual + 128, residual + 128));
...
void CoarseDistanceMap::makeDistanceMap(std::vector<FrameHessian *> frameHessians, FrameHessian *frame) {
    ...
    for (FrameHessian *fh : frameHessians) {
        ...
        for (PointHessian *ph : fh->pointHessians) {
            ...
            if (!(u > 0 && v > 0 && u < w_[1] && v < h_[1])) continue;
