...
class FullSystem {
   public:
    ...
    void addActiveFrame(const std::vector<Vec7> &new_imu_data, ImageAndExposure *image, int incoming_id);
   ...
   private:
    ...
    std::vector<VecX> getNullspaces(std::vector<VecX> &nullspaces_pose, std::vector<VecX> &nullspaces_scale, std::vector<VecX> &nullspaces_affA,
