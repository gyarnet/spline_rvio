...
void Undistort::applyBlurNoise(float *img) const {
    ...
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++) {
            float xBlur = getInterpolatedElement11BiCub(noiseMapX, 4 + (x / (float)w) * benchmark_noiseGridsize, 4 + (y / (float)h) * benchmark_noiseGridsize,
    ...
    for (int x = 0; x < w; x++)
        for (int y = 0; y < h; y++) {
            float yBlur = getInterpolatedElement11BiCub(noiseMapY, 4 + (x / (float)w) * benchmark_noiseGridsize, 4 + (y / (float)h) * benchmark_noiseGridsize,
                                                        benchmark_noiseGridsize + 8);
...
void Undistort::readFromFile(const char *configFileName, int nPars, std::string prefix) {
    ...
    } else if (nPars == 8)  // KB, equi & radtan model
    {
        ...
        if (std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1], &parsOrg[2], &parsOrg[3], &parsOrg[4], &parsOrg[5], &parsOrg[6], &parsOrg[7]) == 8 &&
            std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2) {
            printf("Input resolution: %d %d\n", wOrg, hOrg);
            printf("In: %s%f %f %f %f %f %f %f %f\n", prefix.c_str(), parsOrg[0], parsOrg[1], parsOrg[2], parsOrg[3], parsOrg[4], parsOrg[5], parsOrg[6],
                   parsOrg[7]);
        } else {
            printf(
                "Failed to read camera calibration (invalid format?)\nCalibration "
    ...
    if (parsOrg[2] < 1 && parsOrg[3] < 1) {
        ...
        // pixel at (0,0) contains the integral over intensity over [0,0]-[1,1],
        // whereas I assume the pixel (0,0) to contain a sample of the intensity ot
        // [0,0], which is best approximated by the integral over
