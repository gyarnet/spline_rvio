...
void EnergyFunctional::setDeltaF(CalibHessian *HCalib) {
    ...
    for (EFFrame *f : frames) {
        ...
        for (EFPoint *p : f->points) p->deltaF = p->data->idepth - p->data->idepth_zero;
    }
...
void EnergyFunctional::getImuHessianCurrentFrame(int fi, CalibHessian *HCalib, MatXX &H, VecX &b, bool &spline_valid, MatXX &J_cst, VecX &r_cst, bool print) {
    assert(fi > 0);
    ...
    spline_valid = (cur_fh->shell->trackingRef == prv_fh->shell) && (-tpf < setting_maxImuInterval);
    ...
    if (spline_valid) {
        ...
        Mat33 rot_c_p_meas = (cur_fh->PRE_camToWorld.inverse() * prv_fh->PRE_camToWorld).rotationMatrix();
        r_cst.segment<3>(0) = SO3(rot_c_p_meas.transpose() * rot_c_p_pred).log();
        Mat33 rot_p_w_evalPT = prv_fh->get_camToWorld_evalPT().rotationMatrix().transpose();
        ...
        if (vel_valid) {
            ...
            if ((nxt_fh->shell->trackingRef == cur_fh->shell) && (-tnf < setting_maxImuInterval)) {
                int nxt_idx = CPARS + 3 + 29 * (fi + 1);
                double tnf2 = tnf * tnf;
                Vec3 d_vel_dso = (1 / tpf) * (prv_fh->PRE_camToWorld.translation() - cur_fh->PRE_camToWorld.translation()) -
                                 (1 / tnf) * (cur_fh->PRE_camToWorld.translation() - nxt_fh->PRE_camToWorld.translation());
                Vec3 d_vel_imu = (tpf * cur_fh->spline_q + tpf2 * cur_fh->spline_c + tnf * nxt_fh->spline_q + 2 * tnf2 * nxt_fh->spline_c).head(3);
                r_cst.segment<3>(3) = d_vel_imu - d_vel_dso;
                J_cst.block<3, 3>(3, prv_idx) = -SCALE_XI_TRANS / tpf * I33;
                J_cst.block<3, 3>(3, cur_idx) = SCALE_XI_TRANS * (1 / tpf + 1 / tnf) * I33;
        ...
        for (int j = 0; j < imu_size; j++) {
            ...
            Vec6 imu_pred;
            imu_pred.head(3) = setting_rot_imu_cam * cur_fh->getSplineR_c_t(tt).transpose() * cur_fh->PRE_worldToCam.rotationMatrix() *
                               (HCalib->getScaleScaled() * cur_fh->getSplineAcc(tt) + HCalib->getG());
...
void EnergyFunctional::getImuHessian(MatXX &H, VecX &b, MatXX &J_cst, VecX &r_cst, CalibHessian *HCalib, std::vector<bool> &is_spline_valid, bool print) {
    if (nFrames == 1) return;
    ...
    if (print) {
        ...
        printf("id: %d ba: %5.2f %5.2f %5.2f bg: %5.2f %5.2f %5.2f\n", fh0->frameID, fh0->imu_bias[0], fh0->imu_bias[1], fh0->imu_bias[2], fh0->imu_bias[3],
               fh0->imu_bias[4], fh0->imu_bias[5]);
    }
    ...
    for (int i = 1; i < nFrames; i++) {
        ...
        MatXX J_cst_i;
        VecX r_cst_i;
        getImuHessianCurrentFrame(i, HCalib, H, b, spline_valid, J_cst_i, r_cst_i, print);
        if (spline_valid) {
    ...
    if (print) {
        printf("scale: %5.3f (%5.3f); roll_pitch: (%6.3f, %6.3f)\n\n", HCalib->getScaleScaled(), SCALE_SCALE * HCalib->sg_zero[0], HCalib->sg_scaled[1],
               HCalib->sg_scaled[2]);
    }
}
...
double EnergyFunctional::calcLEnergyF_MT() {
    ...
    double E = 0;
    for (EFFrame *f : frames) E += f->delta_prior.cwiseProduct(f->prior).dot(f->delta_prior);
...
EFFrame *EnergyFunctional::insertFrame(FrameHessian *fh, CalibHessian *HCalib) {
    ...
    for (EFFrame *fh2 : frames) {
        ...
        if (fh2 != eff) connectivityMap[(((uint64_t)fh2->frameID) << 32) + ((uint64_t)eff->frameID)] = Eigen::Vector2i(0, 0);
    }
...
void EnergyFunctional::marginalizeFrame(EFFrame *fh, CalibHessian *HCalib) {
    ...
    if (setting_enable_imu) {
        ...
        getImuHessianCurrentFrame(fh->idx + 1, HCalib, HM_change, bM_change, spline_valid, J_cst, r_cst, false);
        delta2.segment<8>(CPARS + 3 + 29 * (fh->idx + 1)) = delta.segment<8>(CPARS + 8 * (fh->idx + 1));
        if (HCalib->scale_trapped) {
            delta2.segment<21>(CPARS + 3 + 29 * (fh->idx + 1) + 8) = frames[fh->idx + 1]->data->state_imu - frames[fh->idx + 1]->data->state_imu_zero;
        }

        if (fh->idx > 0) {
            // connection from fh->idx-1 to fh->idx
            getImuHessianCurrentFrame(fh->idx, HCalib, HM_change, bM_change, spline_valid, J_cst, r_cst, false);
            delta2.segment<8>(CPARS + 3 + 29 * (fh->idx - 1)) = delta.segment<8>(CPARS + 8 * (fh->idx - 1));
            if (HCalib->scale_trapped) {
                delta2.segment<21>(CPARS + 3 + 29 * (fh->idx - 1) + 8) = frames[fh->idx - 1]->data->state_imu - frames[fh->idx - 1]->data->state_imu_zero;
            }
    ...
    HMScaled.topLeftCorner(ndim, ndim).noalias() -= bli * HMScaled.bottomLeftCorner(step, ndim);
    ...
    for (EFFrame *f : frames) {
        for (int i = 0; i < (int)f->points.size(); i++) {
            ...
            if (p->stateFlag == EFPointStatus::PS_MARGINALIZE) {
                ...
                for (EFResidual *r : p->residualsAll)
                    if (r->isActive()) connectivityMap[(((uint64_t)r->host->frameID) << 32) + ((uint64_t)r->target->frameID)][1]++;
...
void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian *HCalib) {
    ...
    if (!setting_enable_imu || HCalib->imu_initialized) {
        ...
        if (setting_enable_imu) {
            ...
            for (int i = 0; i < nFrames; i++) {
                ...
                if (HCalib->scale_trapped) {
                    delta2.segment<21>(CPARS + 3 + 29 * i + 8) = frames[i]->data->state_imu - frames[i]->data->state_imu_zero;
                }
            }
            delta = delta2.eval();
        }
    ...
    for (int i = 0; i < dim; i++) HFinal_top(i, i) *= (1 + lambda);
    ...
    if (imu_valid) {
        ...
        for (int i = 0; i < nFrames; i++) {
            ...
            HFinal_top.block(0, vi, dim, vs) = HFinal_top.block(0, fi, dim, vs).eval();
            HFinal_top.block(vi, 0, vs, dim) = HFinal_top.block(fi, 0, vs, dim).eval();
            bFinal_top.segment(vi, vs) = bFinal_top.segment(fi, vs).eval();
            vi += vs;
        }
        int fi = CPARS + 3 + 29 * nFrames;
        HFinal_top.block(0, vi, dim, cdim) = HFinal_top.block(0, fi, dim, cdim).eval();
        HFinal_top.block(vi, 0, cdim, dim) = HFinal_top.block(fi, 0, cdim, dim).eval();
    ...
    VecX x = SVecI.asDiagonal() * HFinalScaled.ldlt().solve(SVecI.asDiagonal() * bFinal_top);
