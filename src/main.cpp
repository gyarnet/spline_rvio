...
class VioNode {
   ...
   public:
    ...
    VioNode(int start_frame, double td_cam_imu, const std::string &calib, const std::string &vignette, const std::string &gamma, bool nomt, int preset,
            int mode);
    ~VioNode();
...
void VioNode::imuMessageCallback(const sensor_msgs::ImuConstPtr &msg) {
    ...
    imu_data.segment<3>(1) << msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z;
    imu_data.tail(3) << msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z;
...
void VioNode::imageMessageCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    while (!imu_queue_.empty() && !img_queue_.empty() && img_queue_.front()->timestamp < imu_queue_.back()[0]) {
        // current image pair
        ...
        if (!cur_imu_data.empty()) {
            ...
            Vec7 last_imu_data =
                ((imu_queue_.front()[0] - cur_img->timestamp) * cur_imu_data.back() + (cur_img->timestamp - cur_imu_data.back()[0]) * imu_queue_.front()) /
            ...
            frame_tt_.push_back(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());

            // reinitialize if necessary
            if (full_system_->initFailed && incoming_id_ < 250) {
                std::vector<IOWrap::Output3DWrapper *> wraps = full_system_->outputWrapper;
                delete full_system_;
...
int main(int argc, char **argv) {
    ...
    if (!nhPriv.getParam("T_imu/data", tfm_imu) || !nhPriv.getParam("rate_hz", imu_rate) || !nhPriv.getParam("accelerometer_noise_density", imu_acc_nd) ||
        !nhPriv.getParam("accelerometer_random_walk", imu_acc_rw) || !nhPriv.getParam("gyroscope_noise_density", imu_gyro_nd) ||
        !nhPriv.getParam("gyroscope_random_walk", imu_gyro_rw) || !nhPriv.getParam("imu_topic", imu_topic) || !nhPriv.getParam("cam_topic", cam_topic) ||
        !nhPriv.getParam("calib", calib) || !nhPriv.getParam("results", results_path)) {
    ...
    VioNode vio_node(start_frame, td_cam_imu, calib, vignette, gamma, nomt, preset, mode);
    ...
    setting_weight_imu.topLeftCorner<3, 3>() /= (imu_acc_nd * imu_acc_nd * imu_rate);
    setting_weight_imu.bottomRightCorner<3, 3>() /= (imu_gyro_nd * imu_gyro_nd * imu_rate);
    ...
    setting_weight_imu_bias.bottomRightCorner<3, 3>() /= (imu_gyro_rw * imu_gyro_rw);
